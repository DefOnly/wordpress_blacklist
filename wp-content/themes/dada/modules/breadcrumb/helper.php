<?php
add_action( 'dada_after_main_css', 'breadcrumb_style' );
function breadcrumb_style() {
    wp_enqueue_style( 'dada-breadcrumb', get_theme_file_uri('/modules/breadcrumb/assets/css/breadcrumb.css'), false, DADA_THEME_VERSION, 'all');
}

if( ! function_exists( 'dada_breadcrumb_template' )  ) {
	function dada_breadcrumb_template() {
        if ( !is_front_page() && !is_home() ) {
		    dada_template_part( 'breadcrumb', 'templates/title' );
        }
	}

	add_action( 'dada_breadcrumb', 'dada_breadcrumb_template' );
}
function dada_breadcrumb_params() {
    $params = array(
        "home"            => esc_html__('Home','dada'),
        "home_link"       => home_url('/'),
        "delimiter"       => '<span class="breadcrumb-default-delimiter"></span>',
        "wrapper_classes" => "dark-bg-breadcrumb aligncenter"
    );
    return apply_filters( 'dada_breadcrumb_params', $params );
}

if( ! function_exists( 'dada_update_header_wrapper_classes' )  ) {
	function dada_update_header_wrapper_classes($classes) {
       // array_push($classes, 'wdt-dark-bg'); // For dark bg enable this line
        return $classes;
	}
	add_filter( 'dada_header_wrapper_classes', 'dada_update_header_wrapper_classes', 10, 1 );
}

function dada_breadcrumb_title() {
    $title = get_the_title( dada_get_page_id() );
	...
	if ( ( is_home() && is_front_page() ) || is_singular('attachment') ) {
        ...
        $title = esc_html__("Search Result for: ",'dada').' '.get_search_query();
    }

    return apply_filters( 'dada_breadcrumb_title', '<h1>'.$title.'</h1>' );
}

function dada_breadcrumbs( $home, $separator ) {
    $output      = '';
    $breadcrumbs = array();
    ...
    if( is_search() ) {
    	$breadcrumbs[] = '<a href="javascript:void(0);">' . esc_html__('Search', 'dada') . '</a>';
    }
    ...
    $filtered_breadcrumbs = apply_filters( 'dada_breadcrumbs', $breadcrumbs );
