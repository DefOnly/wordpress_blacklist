<?php
add_action( 'dada_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'dada-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, DADA_THEME_VERSION, 'all');
    }
}

add_filter( 'dada_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = dada_404_page_params();
...
function dada_404_page_params() {
    ...
    return apply_filters( 'dada_404_page_params', $params );
}