...
if( !function_exists('dada_archive_blog_post_misc_defaults') ) {
    function dada_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('dada_archive_blog_post_params_default') ) {
    function dada_archive_blog_post_params_default() {
        $params = array(
            ...
            'archive_meta_elements'  => array( 'tag', 'date' ),
            'archive_readmore_text'  => esc_html__('Read More', 'dada'),
            'enable_excerpt_text'	 => 1,
            'archive_excerpt_length' => 24,
...
if( !function_exists('dada_archive_blog_post_defaults_filter') ) {
    function dada_archive_blog_post_defaults_filter() {
        $defaults = dada_archive_blog_post_defaults();
        return apply_filters( 'dada_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('dada_archive_blog_post_misc_defaults_filter') ) {
    function dada_archive_blog_post_misc_defaults_filter() {
        $defaults = dada_archive_blog_post_misc_defaults();
        return apply_filters( 'dada_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('dada_archive_blog_post_params') ) {
    function dada_archive_blog_post_params() {
        $params = dada_archive_blog_post_params_default();
        return apply_filters( 'dada_archive_blog_post_params', $params );
    }
}

if( !function_exists('dada_get_archive_post_combine_class') ) {
	function dada_get_archive_post_combine_class() {

        $blog_defaults = dada_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'dada_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('dada_get_archive_post_holder_class') ) {
	function dada_get_archive_post_holder_class() {

        $blog_defaults = dada_archive_blog_post_defaults_filter();
        $blog_misc_defaults = dada_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'dada_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('dada_get_archive_post_style') ) {
	function dada_get_archive_post_style() {
		...
		return apply_filters( 'dada_get_archive_post_style', $post_style );
	}
}

if( !function_exists('dada_get_archive_post_column') ) {
	function dada_get_archive_post_column() {
		...
		return apply_filters( 'dada_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'dada_archive_set_post_class', 10, 3);
if( !function_exists('dada_archive_set_post_class') ) {
    function dada_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
            ...
            $blog_params = dada_archive_blog_post_params();
...
add_action( 'dada_after_main_css', 'dada_blog_enqueue_css', 10 );
if( !function_exists( 'dada_blog_enqueue_css' ) ) {
	function dada_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, DADA_THEME_VERSION, 'all');
		...
		wp_enqueue_style( 'jquery-bxslider', get_theme_file_uri('/modules/blog/assets/css/jquery.bxslider.css'), false, DADA_THEME_VERSION, 'all' );
	}
}

add_action( 'dada_before_enqueue_js', 'dada_blog_enqueue_js' );
if( !function_exists( 'dada_blog_enqueue_js' ) ) {
	function dada_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = dada_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'dada_blog_archive_pagination', dada_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'dada_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'dada_excerpt' ) ) {
	function dada_excerpt( $limit = NULL ) {
