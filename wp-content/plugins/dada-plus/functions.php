...
if( !function_exists('dada_plus_get_template_plugin_part') ) {
    function dada_plus_get_template_plugin_part( $file_path, $module, $template, $slug ) {

        $html             = '';
        $template_path    = DADA_PLUS_DIR_PATH . 'modules/' . esc_attr($module);
    ...
    add_filter( 'dada_get_template_plugin_part', 'dada_plus_get_template_plugin_part', 10, 4 );
}

if( !function_exists( 'dada_customizer_panel_priority' ) ) {
    /**
     *  Get : Customizer Panel Priority based on panel name.
     */
    function dada_customizer_panel_priority( $panel ) {
        $priority = 10;
        ...
        return apply_filters( 'dada_customizer_panel_priority', $priority, $panel );
    }
}

if( !function_exists( 'dada_customizer_settings' ) ) {
    /**
     * Get : Customizer settings value
     */
    function dada_customizer_settings( $option ) {
        $settings = get_option( DADA_CUSTOMISER_VAL, array() );
        $settings = isset( $settings[ $option ] ) ? $settings[ $option ] : false;
        return $settings;
    }
}

if( !function_exists( 'dada_customizer_dynamic_style') ) {
    /**
     * Get : Generate style based on selector and property
     */
    function dada_customizer_dynamic_style( $selectors, $properties ) {
        $output = '';
        if( !empty( $selectors ) && !empty( $properties ) ) {
            ...
            $output .= ' { ' . dada_html_output($properties) . ' } ' . "\n";
        }
        return $output;
    }
}
...
if( !function_exists( 'dada_customizer_responsive_typography_settings' ) ) {

    /**
     * Get : Typography Responsive CSS based on option and responsive mode.
     */
    function dada_customizer_responsive_typography_settings( $option, $mode = 'tablet' ) {
        $css = '';
...
if( !function_exists( 'dada_customizer_typography_settings' ) ) {
    /**
     * Get : Typography CSS based on option.
     */
    function dada_customizer_typography_settings( $option ) {
...
if( !function_exists( 'dada_customizer_frontend_font' ) ) {
    /**
     * Load fonts in frontend
     */
    function dada_customizer_frontend_font( $settings, $fonts ) {
        $font = '';
...
if( !function_exists( 'dada_customizer_color_settings' ) ) {
    function dada_customizer_color_settings( $color ) {
        $css = '';
...
if( !function_exists( 'dada_customizer_bg_color_settings' ) ) {
    function dada_customizer_bg_color_settings( $color ) {
        $css = '';
...
if( !function_exists( 'dada_customizer_bg_settings' ) ) {
    function dada_customizer_bg_settings( $bg ) {
        $css = '';
...
# Field Sanitization
if(!function_exists('dada_sanitization')) {
	function dada_sanitization($data) {
		if ( is_array( $data ) && !empty( $data ) ) {
			foreach ( $data as $key => &$value ) {
				if ( is_array( $value ) ) {
					$data[$key] = dada_sanitization($value);
				} else {
