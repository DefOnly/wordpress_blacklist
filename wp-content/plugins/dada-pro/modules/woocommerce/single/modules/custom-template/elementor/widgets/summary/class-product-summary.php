...
class Dada_Shop_Widget_Product_Summary extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Product Single - Summary', 'dada-pro' );
	}

	public function get_style_depends() {
		$styles = apply_filters( 'dada_shop_woo_single_summary_styles',  array ( 'wdt-shop-product-single-summary' ) );
		return $styles;
	}

	public function get_script_depends() {
		$scripts = apply_filters( 'dada_shop_woo_single_summary_scripts',  array ( 'wdt-shop-product-single-summary' ) );
		return $scripts;
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'product_summary_section', array(
			...
			$this->add_control( 'product_id', array(
				'label'       => esc_html__( 'Product Id', 'dada-pro' ),
				'type'        => Controls_Manager::TEXT,
				'description' => esc_html__('Provide product id for which you have to display product summary items. No need to provide ID if it is used in Product single page.', 'dada-pro'),
			) );
			...
			$repeater->add_control( 'summary_item', array(
				'label'       => esc_html__( 'Summary Item', 'dada-pro' ),
				'type'        => Controls_Manager::SELECT,
				'default'     =>'title',
				'options'     => apply_filters( 'dada_shop_woo_single_summary_options',
					array (
						'title'              => esc_html__('Summary Title', 'dada-pro'),
						'rating'             => esc_html__('Summary Rating', 'dada-pro'),
						'price'              => esc_html__('Summary Price', 'dada-pro'),
						'excerpt'            => esc_html__('Summary Excerpt', 'dada-pro'),
						'addtocart'          => esc_html__('Summary Add To Cart', 'dada-pro'),
						'buttons'            => esc_html__('Summary Buttons', 'dada-pro'),
						'meta'               => esc_html__('Summary Meta', 'dada-pro'),
						'meta_sku'           => esc_html__('Summary Meta SKU', 'dada-pro'),
						'meta_category'      => esc_html__('Summary Meta Category', 'dada-pro'),
						'meta_tag'           => esc_html__('Summary Meta Tag', 'dada-pro'),
						'meta_brand'         => esc_html__('Summary Meta Brand', 'dada-pro'),
						'additional_content' => esc_html__('Summary Additional Content', 'dada-pro'),
						'separator1'         => esc_html__('Summary Separator 1', 'dada-pro'),
						'separator2'         => esc_html__('Summary Separator 2', 'dada-pro'),
						'clear1'         => esc_html__('Summary Clear 1', 'dada-pro'),
						'clear2'         => esc_html__('Summary Clear 2', 'dada-pro')
					)
				)
	        ) );
			...
			$this->add_control(
				'items',
				array (
					'label' => esc_html__( 'Items', 'dada-pro' ),
			...
			$repeater->add_control( 'button_item', array(
				'label'       => esc_html__( 'Button Item', 'dada-pro' ),
				'type'        => Controls_Manager::SELECT,
				'default'     =>'',
				'options'     => apply_filters( 'dada_shop_woo_single_summary_button_options',
					array (
						''          => esc_html__('None', 'dada-pro'),
						'btaddtocart'  => esc_html__('Button Add to Cart', 'dada-pro'),
						'wishlist'  => esc_html__('Button Wishlist', 'dada-pro'),
						'compare'   => esc_html__('Button Compare', 'dada-pro'),
						'buynow' 	=> esc_html__('Button Buy Now', 'dada-pro')
					)
				)
	        ) );
			...
			$this->add_control(
				'button_items',
				array (
					'label' => esc_html__( 'Button Items', 'dada-pro' ),
					'type' => Controls_Manager::REPEATER,
					'fields' => $repeater->get_controls(),
					'default' =>  array (),
			...
			$this->add_control( 'content', array(
				'label' => esc_html__( 'Additional Content', 'dada-pro' ),
				'type'  => Controls_Manager::WYSIWYG,
			) );

			$this->add_control( 'alignment', array(
				'label'   => esc_html__( 'Alignment', 'dada-pro' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '',
				'options' => array(
					''            => esc_html__('Left', 'dada-pro'),
					'aligncenter' => esc_html__('Center', 'dada-pro'),
					'alignright'  => esc_html__('Right', 'dada-pro'),
				),
				'description' => esc_html__( 'Choose alignment you would like to use for your product summary.', 'dada-pro' ),
			) );

			$this->add_control( 'button_style', array(
				'label'   => esc_html__( 'Button Style', 'dada-pro' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'simple',
				'options' => array(
			...
			$this->add_control( 'button_radius', array(
				'label'   => esc_html__( 'Button Radius', 'dada-pro' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '',
				'options' => array(
            ...
            $this->add_control( 'button_apply_design_to_cart', array(
				'label'        => esc_html__( 'Button Apply Design to Cart', 'dada-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'yes', 'dada-pro' ),
				'label_off'    => esc_html__( 'no', 'dada-pro' ),
				'default'      => '',
				'return_value' => 'true',
				'description'  => esc_html__( 'Use this option if you want to apply button design to Cart button also.', 'dada-pro' ),
			) );

			$this->add_control( 'button_inline_alignment', array(
				'label'        => esc_html__( 'Button Inline Alignment', 'dada-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'yes', 'dada-pro' ),
				'label_off'    => esc_html__( 'no', 'dada-pro' ),
				'default'      => '',
			...
			$this->add_control( 'button_hide_text', array(
				'label'        => esc_html__( 'Button Hide Text', 'dada-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'yes', 'dada-pro' ),
				'label_off'    => esc_html__( 'no', 'dada-pro' ),
				'default'      => 'false',
			...
			$this->add_control( 'meta_inline_alignment', array(
				'label'        => esc_html__( 'Meta Inline Alignment', 'dada-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'yes', 'dada-pro' ),
				'label_off'    => esc_html__( 'no', 'dada-pro' ),
				'default'      => '',
			...
			$this->add_control(
				'class',
				array (
					'label' => esc_html__( 'Class', 'dada-pro' ),
					'type'  => Controls_Manager::TEXT
				)
			);
		...
		if($settings['product_id'] != '' && $settings['items'] != '') {
			...
			if( is_array( $sub_modules ) && !empty( $sub_modules ) ) {
				foreach( $sub_modules as $sub_module ) {
					...
					if( $file_content = dada_woo_locate_file( $sub_module ) ) {
				...
				$output .= '<div class="woocommerce single-product">';
					...
					$output .= '<div class="wdt-product-summary summary entry-summary '.esc_attr($settings['class']).' '.esc_attr($settings['alignment']).'">';
						...
						if(in_array('countdown', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_sale_countdown_timer' ) ) {
								dada_shop_products_sale_countdown_timer();
							}
						...
						if(in_array('buttons', $items)) {
							...
							if(in_array('wishlist', $button_items)) {
								ob_start();
								do_action( 'dada_woo_loop_product_button_elements_wishlist' );
								$wishlist = ob_get_clean();
								$wishlist = $wishlist;
							}
							if(in_array('compare', $button_items)) {
								ob_start();
								do_action( 'dada_woo_loop_product_button_elements_compare' );
								$compare = ob_get_clean();
								$compare = $compare;
							}
							if(in_array('quickview', $button_items)) {
								ob_start();
								do_action( 'dada_woo_loop_product_button_elements_quickview' );
								$quickview = ob_get_clean();
								$quickview = $quickview;
							}
							if(in_array('sizeguide', $button_items)) {
								ob_start();
								do_action( 'dada_woo_loop_product_button_elements_sizeguide' );
							...
							if(in_array('buynow', $button_items)) {
								ob_start();
								do_action( 'dada_woo_loop_product_button_elements_buynow' );
								$buynow = ob_get_clean();
								$buynow = $buynow;
							}
								...
								echo '<div class="wc_inline_buttons">';
									...
									foreach ($button_items as $key => $value) {
										if($value == 'btaddtocart') {
											echo '<div class="wcwl_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Add to Cart', 'dada-pro').'">';
												echo $$value;
											echo '</div>';
										} else {
											echo $$value;
						...
						if(in_array('meta_sku', $items)) {
							if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
								$meta_data_sku = ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'dada-pro' );
								$meta_sku = '<div class="product_meta_wrapper '.esc_attr($meta_inline_alignment_cls).'"><div class="product_meta"><span class="sku_wrapper"><strong>'.esc_html__( 'SKU:', 'dada-pro' ).'</strong><span class="sku"> ' . $meta_data_sku . ' </span></span></div></div>';
							}
						}
						...
						if(in_array('meta_category', $items)) {
							$meta_category = '<div class="product_meta_wrapper '.esc_attr($meta_inline_alignment_cls).'"><div class="product_meta">'.wc_get_product_category_list( $product->get_id(), ', ', '<span class="posted_in"><strong>' . _n( 'Category:', 'Categories:', count( $product->get_category_ids() ), 'dada-pro' ) . '</strong> ', '</span>' ).'</div></div>';
						}

						// Meta Tag
						$meta_tag = '';
						if(in_array('meta_tag', $items)) {
							$meta_tag = '<div class="product_meta_wrapper '.esc_attr($meta_inline_alignment_cls).'"><div class="product_meta">'.wc_get_product_tag_list( $product->get_id(), ', ', '<span class="tagged_as"><strong>' . _n( 'Tag:', 'Tags:', count( $product->get_tag_ids() ), 'dada-pro' ) . '</strong> ', '</span>' ).'</div></div>';
						}
						...
						// Share / Follow
						$share_follow = '';
						if(in_array('share_follow', $items) && function_exists('dada_shop_single_product_sociable_share_follow')) {
							$share_follow = dada_shop_single_product_sociable_share_follow($settings['product_id'], $settings['share_follow_type'], $settings['social_icon_style'], $settings['social_icon_radius'], $settings['social_icon_inline_alignment']);
						}
						...
						if(in_array('additional_info', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_additional_info' ) ) {
								dada_shop_products_additional_info();
							}
						...
						if(in_array('additional_info_delivery_period', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_ai_delivery_period' ) ) {
								dada_shop_products_ai_delivery_period(true, $product_ai_delivery_period);
							}
						...
						if(in_array('additional_info_realtime_visitors', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_ai_realtime_visitors' ) ) {
								dada_shop_products_ai_realtime_visitors(true, $product_ai_visitors_min_value, $product_ai_visitors_max_value);
							}
						...
						if(in_array('additional_info_shipping_offer', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_ai_shipping_offer' ) ) {
								dada_shop_products_ai_shipping_offer(true);
							}
						...
						if(in_array('additional_info_weight', $items)) {
							$additional_info_weight = '<div class="product_additional_info_wrapper"><div class="product_additional_info"><span class="weight_wrapper"><strong>'.esc_html__('Weight:', 'dada-pro').'</strong><span class="product_weight"> ' . esc_html( $product->get_weight() ) .  esc_html( get_option('woocommerce_weight_unit') ). ' </span></span></div></div>';
						}
						...
						if(in_array('additional_info_dimensions', $items)) {
							...
							if ( $product->has_dimensions() ) {
								$additional_info_dimensions = '<div class="product_additional_info_wrapper"><div class="product_additional_info"><span class="dimensions_wrapper"><strong>'.esc_html__('Dimensions:', 'dada-pro').'</strong><span class="product_dimensions"> ' . esc_html( $dimensions ) . ' </span></span></div></div>';
							}
						}
						...
						if(in_array('additional_info_color', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_ai_color' ) ) {
								dada_shop_products_ai_color(true);
							}
						...
						if(in_array('additional_info_size', $items)) {
							ob_start();
							if ( function_exists( 'dada_shop_products_ai_size' ) ) {
								dada_shop_products_ai_size(true);
							}
			...
			wp_register_style( 'dada-woo-product-summary', '', array (), DADA_PRO_VERSION, 'all' );
			wp_enqueue_style( 'dada-woo-product-summary' );
			...
			if( !is_shop() && !is_product_category() && !is_product_tag() && !is_product() && !is_cart() && !is_checkout() ) {
				...
				$css_file_path = DADA_MODULE_DIR . '/woocommerce/assets/css/common.css';
				...
				$css_file_path = DADA_MODULE_DIR . '/woocommerce/single/assets/css/common.css';
			...
			if( !empty($css) ) {
				wp_add_inline_style( 'dada-woo-product-summary', $css );
			}


		} else {

			$output .= esc_html__('Please provide product id to display corresponding data!', 'dada-pro');
