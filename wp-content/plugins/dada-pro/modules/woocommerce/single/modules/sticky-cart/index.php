...
if( !class_exists( 'Dada_Shop_Single_Module_Sticky_Cart' ) ) {

    class Dada_Shop_Single_Module_Sticky_Cart {
        ...
        function __construct() {
            ...
            // CSS
                add_filter( 'dada_woo_css', array( $this, 'woo_css'), 10, 1 );

            // JS
                add_filter( 'dada_woo_js', array( $this, 'woo_js'), 10, 1 );
        ...
        */
            ...
            function module_dir_path() {
                ...
                if( dada_is_file_in_theme( __FILE__ ) ) {
                    return DADA_MODULE_DIR . '/woocommerce/single/modules/sticky-cart/';
                } else {
            ...
            function module_dir_url() {
                ...
                if( dada_is_file_in_theme( __FILE__ ) ) {
                    return DADA_MODULE_URI . '/woocommerce/single/modules/sticky-cart/';
                } else {
                    ...
                    echo '<div class="container">';
                        echo '<div class="wdt-shop-single-sticky-addtocart-content">';
                            ...
                            echo '<div class="wdt-shop-single-sticky-addtocart-info">';
                                echo '<h3>'.dada_html_output($product->get_name()).'</h3>';
                                echo wc_get_rating_html( $product->get_average_rating() );
                            echo '</div>';
                        echo '</div>';
                        echo '<div class="wdt-shop-single-sticky-addtocart-section">';
                            echo '<span class="wdt-shop-single-sticky-addtocart-price">'.dada_html_output($product->get_price_html()).'</span>';
                            if(dada_check_item_is_in_cart( $product->get_id() )) {
                                echo '<span class="wdt-shop-single-sticky-addtocart-added">'.esc_html__('Product Added To Cart', 'dada-pro').'</span>';
                            } else {
...
if( !function_exists('dada_shop_single_module_sticky_cart') ) {
	function dada_shop_single_module_sticky_cart() {
		return Dada_Shop_Single_Module_Sticky_Cart::instance();
	}
}

dada_shop_single_module_sticky_cart();