...
class WeDesignTech_Common_Controls_Repeater_Contents {
	...
	public function get_controls($elementor_object) {
		...
		$elementor_object->start_controls_section( 'wdt_section_content', array(
			...
			if(in_array('sub_heading', $this->option_default_keys)) {
				$repeater->add_control( 'item_sub_heading', array(
					'label'       => $this->options_default['sub_heading'],
					'type'        => \Elementor\Controls_Manager::TEXT,
					'label_block' => true,
					'placeholder' => esc_html__( 'Item Sub Heading', 'wdt-elementor-addon' ),
					'default'     => esc_html__( 'Item Sub Heading', 'wdt-elementor-addon' ),
		...
		if(is_array($content_positions) && !empty($content_positions)) {
			...
			foreach($content_positions as $content_position) {
				if($content_position == 'image_or_icon') {
					...
					$output .= $this->render_icon($key, $item, $widget_object, $link_start, $link_end);
	...
	public function render_icon($key, $item, $widget_object, $link_start='', $link_end='') {
		$output = '';
