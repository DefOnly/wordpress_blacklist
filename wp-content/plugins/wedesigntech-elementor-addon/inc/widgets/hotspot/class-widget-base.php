			...
			if( is_array($settings['hotspots']) && !empty($settings['hotspots']) ) {
				$output .= '<div class="wdt-hotspot-items-holder">';
					foreach ( $settings['hotspots'] as $key => $hotspot ) {
						...
						if($hotspot['media_image']['url']) {
							$link_start = $link_end = '';
							$image_item = array ();
							$image_item['media_image'] = $hotspot['media_image'];
							$image_item['image_size'] = 'full';
							$image_item['image_custom_dimension'] = '';
							$tooltip_content .= '<div class="wdt-hotspot-item-image">';
								$tooltip_content .= $this->cc_repeater_contents->render_image($image_item, $link_start,$link_end);
							$tooltip_content .= '</div>';
						} else {
							$tooltip_content .= '<div class="wdt-hotspot-item-default">';
								$tooltip_content .= '<i class="fas fa-plus"></i>';
							$tooltip_content .= '</div>';
						}
						...
						$output .= '<'.esc_attr($hotspot_element_tag).' '.$widget_object->get_render_attribute_string( $hotspot_repeater_setting_key ).'>';
							$output .= '<div '.$widget_object->get_render_attribute_string( $hotspot_repeater_trigger_setting_key ).'>';
								...
								if($settings['hotspot_type'] == 'icon') {
									...
									$output .= '<div class="wdt-hotspot-item-default">';
										if($settings['hotspot_animation'] == 'wdt-hotspot-ripple') {
											$output .= '<div class="ripples-blobbe">';
												$output .= '<span class="ripp-item"></span>';
												$output .= '<span class="ripp-item"></span>';
												$output .= '<span class="ripp-item"></span>';
												$output .= '<span class="ripp-item"></span>';
											$output .= '</div>';
